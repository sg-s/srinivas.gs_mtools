...
%
%   arrow(P1,P2,S,W,H,IP,ALPHA,BETA) can be used to specify properties
...
%                          Color Equivalencies
%     -------------------------------------------------------------
%     ColorOrder     arrow         |     Simulink       arrow
%     ----------     ----------     |     ----------     ----------
...
%   Plotting lines with multiple linewidths is slower still.  arrow
%   chooses renderers that produce the best screen images; exported
%   or printed plots may benefit from different choices.
%
%   arrow(P1,P2,S,W,H,'cone',...) will plot cones with bases centered
...
%   option.
%
%   HN = arrow(P1,P2,...) returns a vector of handles to line and
%   surface objects created by arrow.
%
%   arrow COLORS will plot a table of named colors with default
%   brightness.  arrow('colors',BETA) will plot a table of named
%   colors with brightness BETA.
%
%   arrow attempts to preserve the appearance of existing axes.  In
%   particular, arrow will not change XYZLim, View, or CameraViewAngle.
%   arrow does not, however, support stretch-to-fill scaling.  AXIS
...
%   (arrow_messes_up_my_plots.html).
%
%   If a particular aspect ratio or variable limit is required, use
%   DASPECT, PBASPECT, AXIS, or XYZLIM commands before calling arrow.
%   Changing limits or aspect ratios after calling arrow may alter the
%   appearance of arrowheads and initial point markers.  arrow sets
...
%   manual for 3D plots.
%
%   arrow UPDATE will restore the appearance of arrowheads and
%   initial point markers that have become corrupted by changes to
%   limits or aspect ratios.  arrow('update',SF) will redraw initial
...
%
%   arrow UPDATE COLORS will update the magnitude coloring of
%   arrowheads, initial point markers, and lines to conform to the
%   current ColorMap.
%
%   HN = arrow('update',...) returns a vector of handles to updated
%   objects.
%
%   EXAMPLES:
%
...
%     arrow([0 0],[1 3])
%     arrow([0 0],[1 2],'-.e')
%     arrow([0 0],[10 10],'--x2',1)
%     arrow(zeros(10,2),50*rand(10,2),'x',1,3)
%     arrow(zeros(10,2),[10*rand(10,1),500*rand(10,1)],'u')
%     arrow(10*rand(10,2),50*rand(10,2),'x',1,[],1)
%
%     % 3D vectors
%     arrow([0 0 0],[1 1 1])
%     arrow(zeros(20,3),50*rand(20,3),'--x1.5',2)
%     arrow(zeros(100,3),50*rand(100,3),'x',1,3)
%     arrow(zeros(10,3),[10*rand(10,1),500*rand(10,1),50*rand(10,1)],'a')
%     arrow(10*rand(10,3),50*rand(10,3),'x',[],[],0)
%
%     % 3D animation
...
%     hold on
%     pbaspect([1.8,1.4,1])
%     arrow(zeros(3),diag([500,1.5,1.5]),'l',0.7,[],0)
%     p=[20*t,u,v]; inc=4:1:length(t);
%     p2=p(inc,:); p1=p(inc-1,:);
%     hn=arrow(p1(1,:),p2(1,:),'0_b',0.7);
%     for i=2:1:length(p1)
%       delete(hn)
%       hn=arrow(p1(i,:),p2(i,:),'0_b',0.7);
%       pause(0.01)
%     end
%     hold off
%
...
%     arrow(p1,p2,'x',2,4,'cone'), hold on
%     plot3(p1(:,1),p1(:,2),p1(:,3)), hold off
%     pause % change cone size
%     arrow('update',[1,2])
%
%     % Just for fun
%     arrow(zeros(100,3),50*rand(100,3),'x',8,4,[],0.95)
...
%     theta=[0:pi/22:pi/2]';
%     arrow(zeros(12,2),[cos(theta),sin(theta)],'1.5o',1.5,[],[],[],0.5)
%
...
%     w=[1,2,3,4]; h=[4,6,4,2];
%     arrow(zeros(4,2),[10*rand(4,1),500*rand(4,1)],'o*/',w,h,0)
%
%     % Magnitude coloring
%     colormap spring
%     arrow(20*randn(20,3),50*randn(20,3),'|',[],[],0)
...
%     arrow(p1,p2,'o'), grid on, hold off
%
%     % SemiLogX plot
...
%     q1=[1e7,1e6,1e5,1e4,1e3,1e7,1e7,1e7,1e7,1e7,1e7,1e6,1e5,1e4,1e3];
%     q2=1e-2*[9,9,9,9,9,7,5,4,3,2,1,1,1,1,1]; p2=[q1',q2'];
%     arrow(p1,p2,'x'), grid on, hold off
%
%     % SemiLogY plot
...
%     set(gca,'LineStyleOrder',{'-','--','-.',':'})
%     arrow(p1,p2,'*',1,[],0), grid on, hold off
%
%     % Color tables
%     arrow('colors')           % default color table
%     arrow('colors',0.3)       % low contrast color table
%     arrow('colors',0.5)       % high contrast color table
%
%     % Update initial point markers and arrowheads
%     % relative to the current PlotBox diagonal
%     arrow('update')           % redraw same size
%     arrow('update',2)         % redraw double size
%     arrow('update',0.5)       % redraw half size
%     arrow('update',[0.5,2,1]) % redraw W half size,
%                                %        H double size, and
%                                %        IP same size
%
%     See also (arrow_examples.html), (arrow_messes_up_my_plots.html).
...
fig=gcf; ax=gca;
if strcmpi(p1,'update'), ad=getappdata(ax,'arrow');    % update
  LocalLogCheck(ax);
  if size(ad,2)<13, error('Invalid AppData'), end
  setappdata(ax,'arrow',[]); sf=[1,1,1]; flag=0;
  if nargin>1
...
if nargin>5 && ~isempty(ip) && strcmpi(ip,'cone')  % cone plot
  ...
  set(ax,'tag','arrowConePlot');
elseif ~any(p), error('P1 cannot equal P2')
end
if ~all(p)
  warning('arrow:ZeroMagnitude','Zero magnitude ignored')
...
if strcmp(get(ax,'nextplot'),'add') && strcmp(get(fig,'nextplot'),'add')
  ...
  if xys, mode='auto';
    ...
    if strcmp(get(ax,'WarpToFill'),'on')
      warning('arrow:WarpToFill',['Stretch-to-fill scaling not ',...
        'supported;\nuse DASPECT or PBASPECT before calling arrow.']);
    end
  end
...
if nargin<3, [c,ls,lw]=LocalValidateCLSW;% default color, linestyle/width
  ...
  elseif c=='o'                                    % ColorOrder
    if length(ColorOrder)
      ...
      if failed, ColorOrderWarning=['Invalid ColorOrder ',...
        ...
        warning('arrow:ColorOrder',ColorOrderWarning)
      else C=c;
      end
    end, c=C;
  ...
  elseif ~sum(vc==c), c='k'; ColorWarning=['Invalid color switch, ',...
    ...
    warning('arrow:Color',ColorWarning)
  end
end
...
if lw=='/'                                         % LineWidthOrder
  if length(LineWidthOrder)
    ...
    warning('arrow:LineWidthOrder',LineWidthOrderWarning)
  end
end
...
setappdata(ax,'arrow',[getappdata(ax,'arrow');p1,p2,c,w,h,ip,a]);
...
set(ax,'DefaultSurfaceTag','arrow','DefaultSurfaceEdgeColor','none');
  ...
  if length(H1)>1
    for i=1:length(H1)                             % update line colors
      ...
      if length(x)>2                               % multiple lines
        warning('arrow:Magnitude',MagnitudeWarning), continue
      end
      m=sqrt((x(1)-x(2))^2+(y(1)-y(2))^2+(z(1)-z(2))^2);
      c=LocalInterp(minM,maxM,map,m); set(H1(i),'color',c);
    end
  elseif length(H1)==1
    warning('arrow:Magnitude',MagnitudeWarning)
  end
  c=LocalInterp(minM,maxM,map,M);
end
set(ax,'ColorOrder',c);                            % update surfaces
ColorOrder=[];
if strcmp(get(ax,'tag'),'arrowConePlot')
     H=arrow(p1,p2,'o' ,w,h,'cone',a);            % update cones
else H=arrow(p1,p2,'o0',w,h,    ip,a);
end, H=[H1(:);H(:)];
set(ax,'nextplot','replace');
...
if n, clf reset                                    % plot color table
  ...
  for i=1:24, subplot(4,6,i); box on
    ...
    if rem(i,6)==1
      ...
      if i==19
        text(0,-0.25,['{\bf\itarrow}  Named Color Table  ',...
            '( \beta = ',num2str(beta),' )']);
      end
    end
  end
